...
- Update `plan.md` only when strategy/scope/architecture/stop condition/truth boundary changes.
- Update `STATUS.md` after green runs, blockers, commits, pushes, resumes, or handoffs.
- Update `DECISIONS.md` when a durable choice is made or superseded.
- Update `/root/clawd/docs/PLAN_INDEX.md` when active path/status/latest milestone changes.
...
## 13.5 Lifecycle update contract

When this plan changes materially, update these together:

- `plan.md` — only the strategic/contract sections that changed.
- `STATUS.md` — current checkpoint and next actions.
- `DECISIONS.md` — durable decision entry if a choice was made.
- `/root/clawd/docs/PLAN_INDEX.md` — active path/status/latest milestone if affected.
- `memory/projects/<project>.md` — concise handoff if future sessions need it.
