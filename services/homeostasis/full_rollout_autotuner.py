...
def tune_homeostasis_policy(*, r7_root: Path | str = DEFAULT_R7_ROOT) -> Dict[str, Any]:
    ...
    avg_uplift = float(shadow.get("average_estimated_uplift", -1.0) or -1.0)
    safety_regressions = int(shadow.get("safety_regression_count", 999) or 999)
    overrun_events = int(simulation5.get("overrun_events", 999) or 999)
