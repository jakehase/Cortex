...
from cortex_server.modules.codec_policy import get_codec_session_telemetry
from cortex_server.modules.cortex_codec import get_codec_debug_view, get_codec_packet_for_session, update_codec_state_for_session
...
def _authenticated_codec_principal(request: Request) -> AuthenticatedMemoryPrincipal:
    ...
    if isinstance(principal, AuthenticatedMemoryPrincipal):
        return principal
    return _authenticated_nexus_principal(request)
...
def _observe_codec_execution_outcome(
    *,
    ...
    artifact = {
        "recorded": False,
        "reason": "principal_scoped_policy_learning_unavailable",
        "variant": variant,
...
def _codec_evaluation_view(session_key: str, *, eval_query: str = "", max_chars: int = 420, history_limit: int = 8) -> Dict[str, Any]:
    ...
    debug["evaluation"] = {
        ...
        "policy": {
            ...
            "reason": "global learned policy state is unavailable to principal-scoped Codec",
        },
    }
    return debug
...
def _authenticated_nexus_principal(
    ...
    try:
        if request is not None:
            return authenticate_memory_request(request)
        return authenticate_memory_headers(headers)
...
async def get_nexus_codec_policy(request: Request, query: Optional[str] = None, session_key: Optional[str] = None):
    ...
    return {
        ...
        "codec_policy": {
            "enabled": bool(NEXUS_CODEC_ENABLED),
            "version": "cortex.codec.policy.principal-compat.v1",
            "session_telemetry": get_codec_session_telemetry(resolved_session_key),
            "principal_isolated_learning": False,
            "degraded": True,
            "reason": "global learned policy state is withheld from principal-scoped compatibility routes",
        },
...
@router.get("/codec/corpus-replay/reexecute", dependencies=[Depends(require_authenticated_memory_principal)])
...
@router.get("/codec/corpus-replay/live-reexecute", dependencies=[Depends(require_authenticated_memory_principal)])
...
@router.get("/codec/corpus-replay/live-reexecute/compare", dependencies=[Depends(require_authenticated_memory_principal)])
...
@router.get("/codec/corpus-replay/scheduler", dependencies=[Depends(require_authenticated_memory_principal)])
...
async def post_nexus_codec_outcome(payload: OutcomeFeedbackRequest, request: Request):
    resolved_session_key = _principal_codec_session_key(_authenticated_codec_principal(request))
    # The historical policy store is process-global. Recording authenticated
    # tenant feedback into it would allow one principal to steer another. Keep
    # the route explicit and fail closed until that store is partitioned.
    codec_out = {
        "recorded": False,
        "reason": "principal_scoped_policy_learning_unavailable",
        "session_key": resolved_session_key,
...
@router.get("/codec/evaluate", dependencies=[Depends(require_authenticated_memory_principal)])
...
async def outcome_feedback(payload: OutcomeFeedbackRequest, request: Request):
    principal = _authenticated_codec_principal(request)
    return {
        "success": True,
        "recorded": False,
        "artifact": {
            "recorded": False,
            "reason": "principal_scoped_outcome_learning_unavailable",
        },
        "codec_policy": {
            "recorded": False,
            "reason": "principal_scoped_policy_learning_unavailable",
...
async def orchestrate_query(
    ...
    principal = _authenticated_codec_principal(request)
    if request.method.upper() == "GET" and not codec_probe:
        raise HTTPException(
            status_code=405,
            detail="GET orchestration is read-only; model execution requires authenticated POST",
        )
    started = datetime.utcnow()
    request_id = getattr(getattr(request, "state", None), "request_id", "") if request is not None else ""
    session_key = _principal_codec_session_key(principal)
