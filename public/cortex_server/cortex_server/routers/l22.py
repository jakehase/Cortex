...
def _semantic_memory_metadata(metadata: Optional[dict]) -> dict:
    """Normalize governance fields and encode nested values for Chroma."""
...
def _memory_request_hash(*, content: str, memory_type: str, tags: List[str], metadata: dict) -> str:
...
def store_memory_record(
    *,
    ...
    tenant = str(tenant_id or "cortex-local").strip() or "cortex-local"
    workspace = str(workspace_id or "default").strip() or "default"
    resolved_type = str(memory_type or "memory")
    resolved_tags = list(tags or [])
    raw_metadata = dict(metadata or {})
    raw_metadata.setdefault("tenant_id", tenant)
    raw_metadata.setdefault("workspace_id", workspace)
    memory_principal_key = str(raw_metadata.get("memory_principal_key") or "").strip() or None
    ...
    if len(normalized_key.encode("utf-8")) > 256:
        raise HTTPException(status_code=422, detail="idempotency_key exceeds byte limit")

    request_hash = _memory_request_hash(
        content=content,
        memory_type=resolved_type,
        tags=resolved_tags,
        metadata=raw_metadata,
    )
    record_metadata = _semantic_memory_metadata(raw_metadata)
    record_metadata.setdefault("type", resolved_type)
    if resolved_tags:
        record_metadata.setdefault("tags", resolved_tags)
    ...
    record_metadata["idempotency_key"] = normalized_key
    ...
    with _STRUCTURED_MEMORY_LOCK:
        ...
        try:
            ...
            if memory_id in existing_ids:
                ...
                if str(durable_metadata.get("idempotency_hash") or "") != request_hash:
                    raise HTTPException(
                        status_code=409,
                ...
                durable_metadata = record_metadata
...
@router.post("/store")
async def l22_store(request: L22StoreRequest, http_request: Request):
    principal = memory_principal_for_request(http_request)
    return store_memory_record(
        ...
        idempotency_key=request_memory_idempotency_key(http_request, request.idempotency_key),
    )


@router.post("/store_novel")
async def l22_store_novel(request: L22NovelStoreRequest, http_request: Request):
    if not request.content.strip():
        raise HTTPException(status_code=400, detail="Content cannot be empty")

    principal = memory_principal_for_request(http_request)
    if request_memory_idempotency_key(http_request, request.idempotency_key):
        raise HTTPException(
            status_code=422,
            detail="idempotent novelty writes are unavailable; use the principal-scoped /l22/store route",
        )
    metadata = scoped_memory_metadata(principal, request.metadata)
...
@router.post("/search")
async def l22_search(request: L22SearchRequest, http_request: Request):
...
async def l22_search_novel(request: L22NovelSearchRequest, http_request: Request):
    principal = memory_principal_for_request(http_request)
