...
def test_structured_memory_readiness_checks_integrity_and_physical_headroom(monkeypatch, tmp_path):
    db_path = tmp_path / "ready.sqlite3"
    monkeypatch.setenv("CORTEX_L22_STRUCTURED_DB", str(db_path))
    monkeypatch.setattr(l22, "_PREALLOCATE_RECOVERY_RESERVE", False)
    monkeypatch.setattr(l22, "_MAX_PHYSICAL_BYTES", 100 * 1024 * 1024)
    monkeypatch.setattr(l22, "_l22_filesystem_available", lambda: 1024 * 1024 * 1024)

    ready = l22.probe_l22_structured_memory_readiness()
    assert ready["ok"] is True
    assert ready["quickCheck"] == "ok"

    monkeypatch.setattr(l22, "_l22_active_physical_usage", lambda: l22._MAX_PHYSICAL_BYTES + 1)
    not_ready = l22.probe_l22_structured_memory_readiness()
    assert not_ready["ok"] is False
    assert "physical quota" in not_ready["error"]
...
def test_codec_pruning_uses_insertion_order_when_timestamps_collide(monkeypatch, tmp_path):
    db_path = tmp_path / "timestamp-collision.sqlite3"
    monkeypatch.setenv("CORTEX_L22_STRUCTURED_DB", str(db_path))
    monkeypatch.setattr(l22, "_MAX_PHYSICAL_BYTES", 100 * 1024 * 1024)
    monkeypatch.setattr(l22, "_CODEC_MAX_SESSIONS", 2)
    monkeypatch.setattr(l22, "_CODEC_MAX_SNAPSHOTS_PER_SESSION", 2)
    ...
    for session in ("session-z", "session-a", "session-m"):
        for revision in range(3):
            l22.store_structured_memory_record(
                ...
                memory_type="codec_state",
                metadata={
                    "codec_session_key": session,
                    "codec_generated_at": generated_at,
                },
            )

    rows = l22.list_structured_memory_records(memory_type="codec_state", limit=100)
    assert {row["lookup_key"] for row in rows} == {"session-a", "session-m"}
    revisions = {
        key: sorted(
            json.loads(row["content"])["revision"]
            for row in rows
            if row["lookup_key"] == key
        )
        for key in {"session-a", "session-m"}
    }
    assert revisions == {"session-a": [1, 2], "session-m": [1, 2]}
