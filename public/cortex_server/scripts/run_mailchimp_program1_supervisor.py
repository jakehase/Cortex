...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Supervisor-enforced Mailchimp Parity Program 1 controller.")
    parser.add_argument("--date", default="2026-04-02")
    parser.add_argument("--corpus-date", default="2026-04-01")
    sub = parser.add_subparsers(dest="command", required=True)
    sub.add_parser("init")
    sub.add_parser("status")
    sub.add_parser("completion-summary")
    sub.add_parser("materialize-seed")
    ...
    if args.command == "materialize-seed":
        _print(harness.materialize_seed_artifacts(program_date=date, corpus_date=corpus_date))
        return 0
    if args.command == "init":
        supervisor.load_or_create_state(date)
        _print(supervisor.reconcile_state(date))
        return 0
