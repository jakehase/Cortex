...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Wait for Mailchimp Program 1 completion and print a human-readable summary.")
    ...
    if not payload.get("all_complete"):
        print(f"Mailchimp Parity Program 1 for {args.date} is not complete yet.")
        return 3
    summary = payload.get("completion_summary") or {}
    compact = summary.get("summary") or {}
    lines = [
        f"Mailchimp Parity Program 1 complete for {args.date}.",
        ...
        f"Phase 1 cases: {compact.get('phase_1_audience_foundation')}.",
        f"Phase 2 cases: {compact.get('phase_2_campaign_execution')}.",
        f"Phase 3 cases: {compact.get('phase_3_reporting_automation')}.",
