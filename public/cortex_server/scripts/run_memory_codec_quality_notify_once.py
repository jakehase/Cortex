...
from cortex_server.benchmarks import memory_codec_quality_supervisor as supervisor


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Wait for memory/codec quality completion and print a human-readable completion summary.")
    ...
    if not payload.get("all_complete"):
        print(f"Memory/codec quality program for {args.date} is not complete yet.")
        return 3

    summary = payload.get("completion_summary") or {}
    compact = summary.get("summary") or {}
    lines = [
        f"Memory/codec quality program complete for {args.date}.",
        ...
        f"Experiment count: {compact.get('experiment_count')}; winner: {compact.get('winner') or 'unknown'}.",
        f"Failure clusters: {compact.get('triage_cluster_count')}.",
