const DEFAULT_POLICIES = [
  { id: 'coupon-studio-policy-1', title: 'Coupon Studio rollout policy', severity: 'medium' },
  { id: 'coupon-studio-policy-2', title: 'Coupon Studio approval policy', severity: 'high' },
  { id: 'coupon-studio-policy-3', title: 'Coupon Studio incident fallback', severity: 'medium' }
];

export function createCouponStudioPolicies(overrides = {}) {
  return DEFAULT_POLICIES.map((policy, index) => ({
    ...
    notes: overrides.notes || 'Coupon Studio policy posture for expansion wave.'
  }));
}

export function validateCouponStudioPolicies(policies = createCouponStudioPolicies()) {
  const issues = [];
...
export function policySummaryCouponStudio(policies = createCouponStudioPolicies()) {
  return {
