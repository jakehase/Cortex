const PROGRAM = {
  ...
  focus: 'Tenant Provisioning coordinates tenant, provisioning programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["tenant","provisioning"],
  metrics: ["tenant","provisioning","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createTenantProvisioningWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'tenant-provisioning-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Tenant Provisioning' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeTenantProvisioning(workspace = createTenantProvisioningWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createTenantProvisioningNarratives(workspace = createTenantProvisioningWorkspace()) {
