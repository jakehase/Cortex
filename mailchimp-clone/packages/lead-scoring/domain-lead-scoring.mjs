const PROGRAM = {
  id: 'lead-scoring',
  name: 'Lead Scoring',
  focus: 'Lead Scoring coordinates lead, scoring programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["lead","scoring"],
  metrics: ["lead","scoring","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createLeadScoringWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'lead-scoring-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Lead Scoring' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeLeadScoring(workspace = createLeadScoringWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createLeadScoringNarratives(workspace = createLeadScoringWorkspace()) {
