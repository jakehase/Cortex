const PROGRAM = {
  ...
  focus: 'Agency Workspace coordinates agency, workspace programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["agency","workspace"],
  metrics: ["agency","workspace","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createAgencyWorkspaceWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'agency-workspace-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Agency Workspace' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeAgencyWorkspace(workspace = createAgencyWorkspaceWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createAgencyWorkspaceNarratives(workspace = createAgencyWorkspaceWorkspace()) {
