const PROGRAM = {
  ...
  focus: "Service Recovery extends the real-repo expansion with service recovery kits, rollback guides, and dependency restoration.",
  themes: ["service", "recovery"],
  metrics: ["incidents", "recovery", "mttr", "handoffs"],
  lanes: ["detect", "stabilize", "recover", "retrospect"]
};

export function createServiceRecoveryWorkspace(workspaceName = 'Wave 6 workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    scorecards: PROGRAM.metrics.map((metric, index) => ({
      ...
      narrative: "Service Recovery" + ' tracks ' + metric + ' for ' + workspaceName + '.'
    })),
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: "service-recovery" + '-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: "Service Recovery" + ' keeps ' + lane + ' execution visible.'
    }))
  };
}

export function summarizeServiceRecovery(workspace = createServiceRecoveryWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createServiceRecoveryNarratives(workspace = createServiceRecoveryWorkspace()) {
