const PROGRAM = {
  ...
  focus: 'Forecast Planner coordinates forecast, planner programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["forecast","planner"],
  metrics: ["forecast","planner","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createForecastPlannerWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'forecast-planner-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Forecast Planner' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeForecastPlanner(workspace = createForecastPlannerWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createForecastPlannerNarratives(workspace = createForecastPlannerWorkspace()) {
