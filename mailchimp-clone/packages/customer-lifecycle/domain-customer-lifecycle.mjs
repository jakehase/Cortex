const PROGRAM = {
  ...
  focus: 'Customer Lifecycle coordinates customer, lifecycle programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["customer","lifecycle"],
  metrics: ["customer","lifecycle","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createCustomerLifecycleWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'customer-lifecycle-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Customer Lifecycle' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeCustomerLifecycle(workspace = createCustomerLifecycleWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createCustomerLifecycleNarratives(workspace = createCustomerLifecycleWorkspace()) {
