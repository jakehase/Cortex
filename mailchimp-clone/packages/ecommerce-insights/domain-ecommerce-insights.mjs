const PROGRAM = {
  ...
  focus: "Ecommerce Insights extends the real-repo expansion with commerce funnel analysis, revenue segmentation, and order cohorts.",
  themes: ["ecommerce", "insights"],
  metrics: ["orders", "gmv", "ltv", "lift"],
  lanes: ["ingest", "segment", "analyze", "share"]
};

export function createEcommerceInsightsWorkspace(workspaceName = 'Wave 6 workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    scorecards: PROGRAM.metrics.map((metric, index) => ({
      ...
      narrative: "Ecommerce Insights" + ' tracks ' + metric + ' for ' + workspaceName + '.'
    })),
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: "ecommerce-insights" + '-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: "Ecommerce Insights" + ' keeps ' + lane + ' execution visible.'
    }))
  };
}

export function summarizeEcommerceInsights(workspace = createEcommerceInsightsWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createEcommerceInsightsNarratives(workspace = createEcommerceInsightsWorkspace()) {
