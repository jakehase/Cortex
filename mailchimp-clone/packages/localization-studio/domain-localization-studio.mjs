const PROGRAM = {
  ...
  focus: 'Localization Studio coordinates localization, studio programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["localization","studio"],
  metrics: ["localization","studio","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createLocalizationStudioWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'localization-studio-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Localization Studio' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeLocalizationStudio(workspace = createLocalizationStudioWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createLocalizationStudioNarratives(workspace = createLocalizationStudioWorkspace()) {
