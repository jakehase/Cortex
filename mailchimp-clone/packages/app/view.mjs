...
export function page(title, actor, body) {
  ...
  <link rel="stylesheet" href="/static/app-shell.css"><script type="module" src="/static/app-shell.jsx"></script></head><body data-page-title="${escapeHtml(title)}" data-authenticated="${actor ? 'true' : 'false'}"><header data-surface="page-header"><h1>${escapeHtml(title)}</h1><div>${identity}</div></header>${nav(actor)}<main id="app-shell" data-surface="page-main">${body}</main></body></html>`;
}
