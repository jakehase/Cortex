...
function appendJobServiceHeartbeat(state, { workerId = 'mailclone-in-process-worker', status = 'running', detail = 'job runtime heartbeat' } = {}) {
  ensureJobOperationalCollections(state);
  const heartbeat = {
    id: createId('jobhb'),
    workerId,
...
export function recordJobServiceHeartbeat(state, options = {}) {
  const heartbeat = appendJobServiceHeartbeat(state, options);
  persistState(state);
  return heartbeat;
}
...
export function runJobs(state, { workerId = 'mailclone-in-process-worker' } = {}) {
  ensureJobOperationalCollections(state);
  appendJobServiceHeartbeat(state, { workerId, status: 'running', detail: 'runJobs invoked for due queue processing' });
  let changed = true;
