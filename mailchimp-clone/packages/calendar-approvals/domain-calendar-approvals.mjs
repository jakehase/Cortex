const PROGRAM = {
  id: "calendar-approvals",
  name: "Calendar Approvals",
  focus: "Calendar Approvals extends the real-repo expansion with calendar-linked review windows, release holds, and launch approvals.",
  themes: ["calendar", "approval"],
  metrics: ["windows", "sla", "blocks", "throughput"],
  lanes: ["plan", "review", "approve", "ship"]
};

export function createCalendarApprovalsWorkspace(workspaceName = 'Wave 6 workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    scorecards: PROGRAM.metrics.map((metric, index) => ({
      ...
      narrative: "Calendar Approvals" + ' tracks ' + metric + ' for ' + workspaceName + '.'
    })),
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: "calendar-approvals" + '-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: "Calendar Approvals" + ' keeps ' + lane + ' execution visible.'
    }))
  };
}

export function summarizeCalendarApprovals(workspace = createCalendarApprovalsWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createCalendarApprovalsNarratives(workspace = createCalendarApprovalsWorkspace()) {
