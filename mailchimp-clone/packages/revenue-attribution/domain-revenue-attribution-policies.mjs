const DEFAULT_POLICIES = [
  { id: "revenue-attribution-policy-1", title: "Revenue Attribution guardrail", severity: 'medium' },
  { id: "revenue-attribution-policy-2", title: "Revenue Attribution approval ring", severity: 'high' },
  { id: "revenue-attribution-policy-3", title: "Revenue Attribution rollback lane", severity: 'medium' }
];

export function createRevenueAttributionPolicies(overrides = {}) {
  return DEFAULT_POLICIES.map((policy, index) => ({
    ...
    notes: overrides.notes || "Revenue Attribution policy pack for the real-repo expansion wave."
  }));
}

export function validateRevenueAttributionPolicies(policies = createRevenueAttributionPolicies()) {
  const issues = [];
...
export function policySummaryRevenueAttribution(policies = createRevenueAttributionPolicies()) {
