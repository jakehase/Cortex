const PROGRAM = {
  id: "predictive-segments",
  name: "Predictive Segments",
  focus: "Predictive Segments extends the real-repo expansion with predictive segment catalogs, signal scoring, and action readiness.",
  themes: ["predictive", "segments"],
  metrics: ["signals", "segments", "confidence", "coverage"],
  lanes: ["collect", "score", "activate", "observe"]
};

export function createPredictiveSegmentsWorkspace(workspaceName = 'Wave 6 workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    scorecards: PROGRAM.metrics.map((metric, index) => ({
      ...
      narrative: "Predictive Segments" + ' tracks ' + metric + ' for ' + workspaceName + '.'
    })),
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: "predictive-segments" + '-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: "Predictive Segments" + ' keeps ' + lane + ' execution visible.'
    }))
  };
}

export function summarizePredictiveSegments(workspace = createPredictiveSegmentsWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createPredictiveSegmentsNarratives(workspace = createPredictiveSegmentsWorkspace()) {
