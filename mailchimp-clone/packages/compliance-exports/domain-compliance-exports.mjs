const PROGRAM = {
  ...
  focus: 'Compliance Exports coordinates compliance, exports programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["compliance","exports"],
  metrics: ["compliance","exports","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createComplianceExportsWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'compliance-exports-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Compliance Exports' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeComplianceExports(workspace = createComplianceExportsWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createComplianceExportsNarratives(workspace = createComplianceExportsWorkspace()) {
