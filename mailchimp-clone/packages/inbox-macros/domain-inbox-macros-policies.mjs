const DEFAULT_POLICIES = [
  { id: 'inbox-macros-policy-1', title: 'Inbox Macros rollout policy', severity: 'medium' },
  { id: 'inbox-macros-policy-2', title: 'Inbox Macros approval policy', severity: 'high' },
  { id: 'inbox-macros-policy-3', title: 'Inbox Macros incident fallback', severity: 'medium' }
];

export function createInboxMacrosPolicies(overrides = {}) {
  return DEFAULT_POLICIES.map((policy, index) => ({
    ...
    notes: overrides.notes || 'Inbox Macros policy posture for expansion wave.'
  }));
}

export function validateInboxMacrosPolicies(policies = createInboxMacrosPolicies()) {
  const issues = [];
...
export function policySummaryInboxMacros(policies = createInboxMacrosPolicies()) {
  return {
