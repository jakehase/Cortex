const PROGRAM = {
  id: 'inbox-macros',
  name: 'Inbox Macros',
  focus: 'Inbox Macros coordinates inbox, macros programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["inbox","macros"],
  metrics: ["inbox","macros","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createInboxMacrosWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'inbox-macros-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Inbox Macros' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeInboxMacros(workspace = createInboxMacrosWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createInboxMacrosNarratives(workspace = createInboxMacrosWorkspace()) {
