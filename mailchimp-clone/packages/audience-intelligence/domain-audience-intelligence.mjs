const PROGRAM = {
  ...
  focus: 'predictive segments, scoring, and lookalike packaging',
  metrics: [
  'scores',
  'segments',
  'models'
...
export function createAudienceIntelligenceWorkspace(workspaceName = 'Demo workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    workstreams: PROGRAM.items.map((item, index) => ({
      id: 'audience-intelligence-ws-' + (index + 1),
      title: item,
...
export function summarizeAudienceIntelligence(workspace = createAudienceIntelligenceWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function validateAudienceIntelligencePlan(plan = {}) {
  const issues = [];
...
export function createAudienceIntelligenceBrief(overrides = {}) {
  return {
