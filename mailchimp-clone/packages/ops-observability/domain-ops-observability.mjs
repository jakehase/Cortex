const PROGRAM = {
  ...
  focus: 'Ops Observability coordinates ops, observability programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["ops","observability"],
  metrics: ["ops","observability","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createOpsObservabilityWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'ops-observability-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Ops Observability' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeOpsObservability(workspace = createOpsObservabilityWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createOpsObservabilityNarratives(workspace = createOpsObservabilityWorkspace()) {
