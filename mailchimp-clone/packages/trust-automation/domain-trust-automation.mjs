const PROGRAM = {
  ...
  focus: "Trust Automation extends the real-repo expansion with trust workflows, control automation, and remediation proof chains.",
  themes: ["trust", "automation"],
  metrics: ["controls", "automation", "exceptions", "proof"],
  lanes: ["map", "automate", "review", "attest"]
};

export function createTrustAutomationWorkspace(workspaceName = 'Wave 6 workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    scorecards: PROGRAM.metrics.map((metric, index) => ({
      ...
      narrative: "Trust Automation" + ' tracks ' + metric + ' for ' + workspaceName + '.'
    })),
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: "trust-automation" + '-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: "Trust Automation" + ' keeps ' + lane + ' execution visible.'
    }))
  };
}

export function summarizeTrustAutomation(workspace = createTrustAutomationWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createTrustAutomationNarratives(workspace = createTrustAutomationWorkspace()) {
