const PROGRAM = {
  ...
  focus: 'Campaign Calendar coordinates campaign, calendar programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["campaign","calendar"],
  metrics: ["campaign","calendar","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createCampaignCalendarWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'campaign-calendar-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Campaign Calendar' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeCampaignCalendar(workspace = createCampaignCalendarWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createCampaignCalendarNarratives(workspace = createCampaignCalendarWorkspace()) {
