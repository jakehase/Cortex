const PROGRAM = {
  ...
  focus: "Attribution Modeling extends the real-repo expansion with attribution modeling, revenue influence mapping, and outcome calibration.",
  themes: ["attribution", "measurement"],
  metrics: ["influence", "spend", "conversion", "lag"],
  lanes: ["baseline", "model", "review", "share"]
};

export function createAttributionModelingWorkspace(workspaceName = 'Wave 6 workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    scorecards: PROGRAM.metrics.map((metric, index) => ({
      ...
      narrative: "Attribution Modeling" + ' tracks ' + metric + ' for ' + workspaceName + '.'
    })),
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: "attribution-modeling" + '-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: "Attribution Modeling" + ' keeps ' + lane + ' execution visible.'
    }))
  };
}

export function summarizeAttributionModeling(workspace = createAttributionModelingWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createAttributionModelingNarratives(workspace = createAttributionModelingWorkspace()) {
