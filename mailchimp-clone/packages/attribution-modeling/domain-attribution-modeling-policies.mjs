const DEFAULT_POLICIES = [
  { id: "attribution-modeling-policy-1", title: "Attribution Modeling guardrail", severity: 'medium' },
  { id: "attribution-modeling-policy-2", title: "Attribution Modeling approval ring", severity: 'high' },
  { id: "attribution-modeling-policy-3", title: "Attribution Modeling rollback lane", severity: 'medium' }
];

export function createAttributionModelingPolicies(overrides = {}) {
  return DEFAULT_POLICIES.map((policy, index) => ({
    ...
    notes: overrides.notes || "Attribution Modeling policy pack for the real-repo expansion wave."
  }));
}

export function validateAttributionModelingPolicies(policies = createAttributionModelingPolicies()) {
  const issues = [];
...
export function policySummaryAttributionModeling(policies = createAttributionModelingPolicies()) {
