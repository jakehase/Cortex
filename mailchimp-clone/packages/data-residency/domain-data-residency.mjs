const PROGRAM = {
  id: 'data-residency',
  name: 'Data Residency',
  focus: 'Data Residency coordinates data, residency programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["data","residency"],
  metrics: ["data","residency","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createDataResidencyWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'data-residency-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Data Residency' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeDataResidency(workspace = createDataResidencyWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createDataResidencyNarratives(workspace = createDataResidencyWorkspace()) {
