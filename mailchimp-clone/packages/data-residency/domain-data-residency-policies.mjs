const DEFAULT_POLICIES = [
  { id: 'data-residency-policy-1', title: 'Data Residency rollout policy', severity: 'medium' },
  { id: 'data-residency-policy-2', title: 'Data Residency approval policy', severity: 'high' },
  { id: 'data-residency-policy-3', title: 'Data Residency incident fallback', severity: 'medium' }
];

export function createDataResidencyPolicies(overrides = {}) {
  return DEFAULT_POLICIES.map((policy, index) => ({
    ...
    notes: overrides.notes || 'Data Residency policy posture for expansion wave.'
  }));
}

export function validateDataResidencyPolicies(policies = createDataResidencyPolicies()) {
  const issues = [];
...
export function policySummaryDataResidency(policies = createDataResidencyPolicies()) {
  return {
