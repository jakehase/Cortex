const DEFAULT_POLICIES = [
  { id: "multi-account-control-policy-1", title: "Multi-Account Control guardrail", severity: 'medium' },
  { id: "multi-account-control-policy-2", title: "Multi-Account Control approval ring", severity: 'high' },
  { id: "multi-account-control-policy-3", title: "Multi-Account Control rollback lane", severity: 'medium' }
];

export function createMultiAccountControlPolicies(overrides = {}) {
  return DEFAULT_POLICIES.map((policy, index) => ({
    ...
    notes: overrides.notes || "Multi-Account Control policy pack for the real-repo expansion wave."
  }));
}

export function validateMultiAccountControlPolicies(policies = createMultiAccountControlPolicies()) {
  const issues = [];
...
export function policySummaryMultiAccountControl(policies = createMultiAccountControlPolicies()) {
