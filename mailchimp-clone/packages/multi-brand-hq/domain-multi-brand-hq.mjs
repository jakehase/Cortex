const PROGRAM = {
  id: 'multi-brand-hq',
  name: 'Multi Brand Hq',
  focus: 'Multi Brand Hq coordinates multi, brand, hq programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["multi","brand","hq"],
  metrics: ["multi","brand","hq","coverage"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createMultiBrandHqWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'multi-brand-hq-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Multi Brand Hq' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeMultiBrandHq(workspace = createMultiBrandHqWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createMultiBrandHqNarratives(workspace = createMultiBrandHqWorkspace()) {
