const DEFAULT_POLICIES = [
  { id: 'multi-brand-hq-policy-1', title: 'Multi Brand Hq rollout policy', severity: 'medium' },
  { id: 'multi-brand-hq-policy-2', title: 'Multi Brand Hq approval policy', severity: 'high' },
  { id: 'multi-brand-hq-policy-3', title: 'Multi Brand Hq incident fallback', severity: 'medium' }
];

export function createMultiBrandHqPolicies(overrides = {}) {
  return DEFAULT_POLICIES.map((policy, index) => ({
    ...
    notes: overrides.notes || 'Multi Brand Hq policy posture for expansion wave.'
  }));
}

export function validateMultiBrandHqPolicies(policies = createMultiBrandHqPolicies()) {
  const issues = [];
...
export function policySummaryMultiBrandHq(policies = createMultiBrandHqPolicies()) {
  return {
