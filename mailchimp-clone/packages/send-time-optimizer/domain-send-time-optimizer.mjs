const PROGRAM = {
  ...
  focus: 'Send Time Optimizer coordinates send, time, optimizer programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["send","time","optimizer"],
  metrics: ["send","time","optimizer","coverage"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createSendTimeOptimizerWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'send-time-optimizer-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Send Time Optimizer' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeSendTimeOptimizer(workspace = createSendTimeOptimizerWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createSendTimeOptimizerNarratives(workspace = createSendTimeOptimizerWorkspace()) {
