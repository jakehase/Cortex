const DEFAULT_POLICIES = [
  { id: 'journey-metrics-policy-1', title: 'Journey Metrics rollout policy', severity: 'medium' },
  { id: 'journey-metrics-policy-2', title: 'Journey Metrics approval policy', severity: 'high' },
  { id: 'journey-metrics-policy-3', title: 'Journey Metrics incident fallback', severity: 'medium' }
];

export function createJourneyMetricsPolicies(overrides = {}) {
  return DEFAULT_POLICIES.map((policy, index) => ({
    ...
    notes: overrides.notes || 'Journey Metrics policy posture for expansion wave.'
  }));
}

export function validateJourneyMetricsPolicies(policies = createJourneyMetricsPolicies()) {
  const issues = [];
...
export function policySummaryJourneyMetrics(policies = createJourneyMetricsPolicies()) {
  return {
