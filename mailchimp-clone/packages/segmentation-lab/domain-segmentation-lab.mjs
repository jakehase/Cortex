const PROGRAM = {
  ...
  focus: 'Segmentation Lab coordinates segmentation, lab programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["segmentation","lab"],
  metrics: ["segmentation","lab","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createSegmentationLabWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'segmentation-lab-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Segmentation Lab' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeSegmentationLab(workspace = createSegmentationLabWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createSegmentationLabNarratives(workspace = createSegmentationLabWorkspace()) {
