const PROGRAM = {
  ...
  focus: 'Support Playbooks coordinates support, playbooks programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["support","playbooks"],
  metrics: ["support","playbooks","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createSupportPlaybooksWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'support-playbooks-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Support Playbooks' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeSupportPlaybooks(workspace = createSupportPlaybooksWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createSupportPlaybooksNarratives(workspace = createSupportPlaybooksWorkspace()) {
