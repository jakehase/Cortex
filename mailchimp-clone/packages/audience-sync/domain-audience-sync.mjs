const PROGRAM = {
  id: 'audience-sync',
  name: 'Audience Sync',
  focus: 'Audience Sync coordinates audience, sync programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["audience","sync"],
  metrics: ["audience","sync","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createAudienceSyncWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'audience-sync-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Audience Sync' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeAudienceSync(workspace = createAudienceSyncWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createAudienceSyncNarratives(workspace = createAudienceSyncWorkspace()) {
