const PROGRAM = {
  ...
  focus: 'Workspace Catalog coordinates workspace, catalog programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["workspace","catalog"],
  metrics: ["workspace","catalog","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createWorkspaceCatalogWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'workspace-catalog-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Workspace Catalog' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeWorkspaceCatalog(workspace = createWorkspaceCatalogWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createWorkspaceCatalogNarratives(workspace = createWorkspaceCatalogWorkspace()) {
