const DEFAULT_POLICIES = [
  { id: 'subscription-ops-policy-1', title: 'Subscription Ops rollout policy', severity: 'medium' },
  { id: 'subscription-ops-policy-2', title: 'Subscription Ops approval policy', severity: 'high' },
  { id: 'subscription-ops-policy-3', title: 'Subscription Ops incident fallback', severity: 'medium' }
];

export function createSubscriptionOpsPolicies(overrides = {}) {
  return DEFAULT_POLICIES.map((policy, index) => ({
    ...
    notes: overrides.notes || 'Subscription Ops policy posture for expansion wave.'
  }));
}

export function validateSubscriptionOpsPolicies(policies = createSubscriptionOpsPolicies()) {
  const issues = [];
...
export function policySummarySubscriptionOps(policies = createSubscriptionOpsPolicies()) {
  return {
