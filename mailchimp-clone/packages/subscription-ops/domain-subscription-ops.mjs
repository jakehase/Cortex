const PROGRAM = {
  ...
  focus: 'Subscription Ops coordinates subscription, ops programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["subscription","ops"],
  metrics: ["subscription","ops","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createSubscriptionOpsWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'subscription-ops-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Subscription Ops' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeSubscriptionOps(workspace = createSubscriptionOpsWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createSubscriptionOpsNarratives(workspace = createSubscriptionOpsWorkspace()) {
