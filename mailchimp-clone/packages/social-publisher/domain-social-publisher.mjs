const PROGRAM = {
  ...
  focus: 'Social Publisher extends the continuation with social, publisher planning, monitoring, and launch operations.',
  themes: ["social","publisher"],
  metrics: ["social","publisher","velocity","coverage"],
  lanes: ["brief","build","ship","review"]
};

export function createSocialPublisherWorkspace(workspaceName = 'Continuation workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    scorecards: PROGRAM.metrics.map((metric, index) => ({
      ...
      narrative: 'Social Publisher tracks ' + metric + ' for ' + workspaceName + '.'
    })),
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'social-publisher-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Social Publisher keeps ' + lane + ' milestones visible.'
    }))
  };
}

export function summarizeSocialPublisher(workspace = createSocialPublisherWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createSocialPublisherNarratives(workspace = createSocialPublisherWorkspace()) {
