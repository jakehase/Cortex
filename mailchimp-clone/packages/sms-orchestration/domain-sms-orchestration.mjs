const PROGRAM = {
  id: 'sms-orchestration',
  name: 'Sms Orchestration',
  focus: 'Sms Orchestration coordinates sms, orchestration programs with reusable playbooks, controls, and rollout telemetry.',
  themes: ["sms","orchestration"],
  metrics: ["sms","orchestration","coverage","latency"],
  lanes: ["plan","stage","launch","stabilize"]
};

export function createSmsOrchestrationWorkspace(workspaceName = 'Expansion workspace') {
  return {
    ...PROGRAM,
    workspaceName,
    ...
    programs: PROGRAM.lanes.map((lane, index) => ({
      id: 'sms-orchestration-program-' + (index + 1),
      lane,
      owner: lane + '-owner',
      status: index === 0 ? 'active' : index === 1 ? 'planned' : index === 2 ? 'review' : 'monitoring',
      narrative: 'Sms Orchestration' + ' keeps ' + lane + ' work visible for stakeholders.'
    }))
  };
}

export function summarizeSmsOrchestration(workspace = createSmsOrchestrationWorkspace()) {
  return {
    id: workspace.id,
    name: workspace.name,
...
export function createSmsOrchestrationNarratives(workspace = createSmsOrchestrationWorkspace()) {
